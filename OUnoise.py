...
参数        含义              数据类型         取值范围    说明
---------   ---------------   --------------   ---------   --------------
#dim         需要的噪声维度    int              >0          默认值1 会根据mu的维度进行调整
#mu          噪声的均值        np向量           --          默认值0 优先以mu的维度为准 1维输入可以为float
#theta       幅度参数          float或np向量    >=0         默认值0.15 调整噪声的连续性 越大越不连续 接近高斯噪声的样子
#sigma       方差参数          float或nd矩阵    >=0         默认值0.2 调整噪声的取值范围 不改变噪声的整体形状 相当于乘一个系数 矩阵就是协方差
#dt          时间间隔          float            >=0         默认值0.01 相同theta dt越大噪声的连续性也越不好 因为时间相关性弱了
#x0          初始取值          np向量           --          默认值为均值mu 噪声的初始值
#rand_num    随机数种子        int              >=0         默认值111 随机数种子决定整个噪声序列的取值 
'''
