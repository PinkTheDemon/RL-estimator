...
class ActorRNN(nn.Module):
    def __init__(self, dim_input, dim_output, dim_fc1=[256], dim_fc2=[256], type_activate='tanh', 
        ...
        for dim_out in dim_fc1 : 
            ...
            if type_activate.lower() == 'relu' : 
                ...
                self.fc1.append(nn.LeakyReLU(negative_slope=0.1))
        ...
        #endregion
        #region 输出层
        self.out = nn.Linear(dim_in, self.dim_output) # L.flatten
        self.weight_init()
        #endregion
    ...
    def weight_init(self) : 
        ...
        if isinstance(self.out, nn.Linear) : 
            nn.init.kaiming_uniform_(self.out.weight, mode="fan_in", nonlinearity=nonlinearity)
...
class RL_estimator(est.Estimator):
    def __init__(self, model:Model, lr, lr_min, nnParams:dict, gamma=1.0, device='cpu', x0_hat=None, P0_hat=None) -> None:
        ...
        dim_output = fun.ds2do(model.dim_state+1) # 一阶SumOfSquares，即增加常数项
        self.device = device
        self.gamma = gamma
    ...
    def reset(self, x0_hat, P0_hat) -> None:
        ...
        if P0_hat is not None:
            # 保持后续计算的统一性给L矩阵转成tensor
            self.L = fun.P3dtoP4d(x_bar=x0_hat, P3d=fun.inv(P0_hat))
    ...
    def estimate(self, y, Q, R, isEval:bool=True):
        ...
        result = est.NLSF_uniform(self.L.detach().squeeze().cpu().numpy(), y_seq=[y], 
                                  Q=Q, R=R, f=self.model.f, h=self.model.h, F=self.model.F, H=self.model.H, 
                                  mode="sumofsquares", x0=[self.x_hat], gamma=self.gamma)
        ...
        if isEval:
            ...
            self.policy.eval()
        self.L, self.hidden = self.policy.forward(input, self.hidden) ## 这里不一定没计算梯度，可能还是要用到torch.no_grad()或者torch.inference_mode()
        self.x_hat = result.x[-ds: ]
        self.y_hat = self.model.h(self.x_hat)
        return self.x_hat, self.L
    ...
    def train(self, x_batch_test:list, y_batch_test:list, trainParams:dict, estParams:dict) -> None:
        ...
        for i in range(len(y_batch)):
            ...
            for t in range(len(y_seq)):
                ...
                x_next_hat, L_next = self.estimate(y, Q, R, isEval=False)
                x_hat_seq.append(x_next_hat)
                y_hat_seq.append(self.model.h(x_next_hat))
                L_seq.append(L_next.detach().squeeze().cpu().numpy())
                #endregion
                #region 计算targetQ和Q
                if t >= 18: # 窗口大于指定长度开始训练（修改：窗口长度小于指定长度的数据不要）## 是不是等大于多一点的窗口再开始训练好一点？
                    for _ in range(trainParams["aver_num"]): # 为了实现函数拟合，取多个值计算arrival cost值
                        ...
                        result = est.NLSF_uniform(L_seq[t-train_window], y_seq=y_list[ :-1], Q=Q, R=R, gamma=self.gamma, 
                                                  f=self.model.f, h=self.model.h, F=self.model.F, H=self.model.H, mode="sumofsquares-end", 
                                                  x0=x_hat_seq[t-train_window:-1], xend=x_next_noise)
                        # end if t(step)
                        min_fun_value = result.fun
                        targetQ = min_fun_value@min_fun_value + (y_list[-1] - self.model.h(x_next_noise))@fun.inv(R)@(y_list[-1] - self.model.h(x_next_noise))
                        Qvalue = self.value(x=x_next_noise, L=L_next)
    ...
    def initialize(self, x_batch_init:list, y_batch_init:list, estParams:dict):
        optimizer = Adam(self.policy.parameters(), lr=1e4)
        scheduler = ReduceLROnPlateau(optimizer, mode='min', patience=50, factor=0.5, min_lr=1e-1, verbose=True)
        ...
        for i in range(len(y_batch_init)):
            ...
            for t in range(len(y_seq)):
                ...
                P4d_inv = fun.P3dtoP4d(x_bar=xtp1, P3d=Ptp1_inv, h3d=c.item())
                target_Pinv_seq.append(P4d_inv)
            input_seq = torch.FloatTensor(np.stack(input_seq)).unsqueeze(0).to(self.device)
            L_seq, _ = self.policy.forward(input_seq, None)
            Pinv_seq = L_seq @ L_seq.permute(*range(L_seq.dim() - 2), -1, -2)
            target_L_seq = torch.FloatTensor(np.stack(target_Pinv_seq)).unsqueeze(0).to(self.device)
            loss = F.mse_loss(L_seq, target_L_seq)
...
def main():
    ...
    #region 策略网络初始化
    initNetName = f"net/SOS_{nnParams['type_rnn']}_{nnParams['type_activate']}_dropout{nnParams['dropout']}_layer{nnParams['num_rnn_layers']}_"\
