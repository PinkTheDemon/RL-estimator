...
def simulate(args, sim_num=1, rand_num=1111, STATUS='EKF') : 
    ...
    for i in range(sim_num) : 
        ...
        if 'MHE' not in STATUS : 
            ...
            hidden = None
            for t in t_seq : 
                try : y_list = y_seq[t:t+args.window]
                except IndexError : 
                    result = result[ds:]
                    x_hat_seq.append(result[ds:2*ds])
                '''非线性最小二乘问题求解'''
                result = est.NLSF(x_hat, P_hat, y_list, args.Q, args.R)
                x_next_hat = result[ds:2*ds]
                x_hat_new = result[:ds]
                if 'EKF' in STATUS : _, P_next_hat = est.EKF(x_hat_new, P_hat, y_seq[t], args.Q, args.R)
                if 'UKF' in STATUS : _, P_next_hat = est.UKF(x_hat_new, P_hat, y_seq[t], args.Q, args.R)
                if 'RLF' in STATUS : 
                    P_inv_next, _, hidden = agent.get_Pinv(x_hat, y_seq[t], hidden)
...
def main() : 
    args = def_param()
    args.hidden_layer = ([500,500,500,500],[500,500,500,500])
    args.num_steps = 8
    args.batch_size = 16
    args.window = 2
    ...
    simulate(args, sim_num=50, rand_num=10086, STATUS='MHE-RLF')
