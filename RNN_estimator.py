...
# 不用全局变量的话，需要修改estimate函数，进而需要修改simulate中对estimate的参数传递，进而需要为所有的estimate方法增加对lb和ub的适配
lb = np.array((-20, -30, 0))
...
class RL_estimator(est.Estimator):
    ...
    def estimate(self, y, Q, R, isEval:bool=True):
        ...
        result = est.NLSF_uniform(self.P_inv.detach().squeeze().cpu().numpy().reshape((ds,-1)), y_seq=[y], 
                                  ...
                                  mode="quadratic", x0=[self.x_hat], x0_bar=self.x_hat, gamma=self.gamma, lb=lb, ub=ub)
    ...
    def load_network(self, baseName) -> None:
        self.policy.load_state_dict(torch.load(baseName+".mdl", weights_only=True))
        self.optimizer.load_state_dict(torch.load(baseName+".opt", weights_only=True))
        self.scheduler.load_state_dict(torch.load(baseName+".sch", weights_only=True))
            ...
            for t in range(len(y_seq)):
                ...
                if t >= train_window: # 大于指定长度开始训练
                    for _ in range(trainParams["aver_num"]): # 为了实现函数拟合，取多个值计算arrival cost值
                        ...
                        # 限定x_next_noise的范围
                        x_next_noise = np.clip(x_next_noise, lb, ub)
                ...
                if len(Q_list) >= trainParams["seq_len"]*trainParams["aver_num"]: # 数据达到指定长度，做一次网络参数更新(修改：截断训练)
                    ...
                    if (lr != self.optimizer.param_groups[0]['lr']) :
                        print(f"learning rate: {lr} -> {self.optimizer.param_groups[0]['lr']}", flush=True)
    ...
    def initialize(self, x_batch_init:list, y_batch_init:list, estParams:dict):
        ...
        for i in range(len(y_batch_init)):
            ...
            for t in range(len(y_seq)):
                ...
                c = self.gamma*c + (y_seq[t].reshape(-1,1) - Ht@Ft@xt).T @ (Rinv - Rinv@Ht@P_hat_seq[t+1]@Ht.T@Rinv) @ (y_seq[t].reshape(-1,1) - Ht@Ft@xt)
...
def main():
    ...
    if os.path.exists(initNetName):
        agent.policy.load_state_dict(torch.load(initNetName, weights_only=True))
    else :
    ...
    #endregion
    # 加载模型
    agent.load_network("net/RNN_net(79)end")
