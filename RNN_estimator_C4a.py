...
class RL_estimator(est.Estimator):
    def __init__(self, model:Continuous4, lr, lr_min, nnParams:dict, gamma=1.0, device='cpu', x0_hat=None, P0_hat=None) -> None:
        self.model = model
        dim_input = model.dim_obs#model.dim_state + 
    ...
    def estimate(self, y, Q, R, isEval:bool=True):
        ...
        input = np.tile(y, (1,1,1))#np.tile(np.hstack((self.x_hat, y)), (1,1,1))
    ...
    def initialize(self, x_batch_init:list, y_batch_init:list, estParams:dict):
        ...
        for _ in range(4):
            for i in range(len(y_batch_init)):
                ...
                # c = 0
                for t in range(len(y_seq)):
                    input_seq.append(y_seq[t])#input_seq.append(np.hstack((x_hat_seq[t], y_seq[t])))
...
def main():
    ...
    #region 策略网络初始化
    initNetName = f"net/c4a{nnParams['type_rnn']}_{nnParams['type_activate']}_dropout{nnParams['dropout']}_layer{nnParams['num_rnn_layers']}_"\
